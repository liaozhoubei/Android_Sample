...
public class MessengerService extends Service {
        ...
        public boolean handleMessage(@NonNull Message msg) {
            switch (msg.what) {
                ...
                case MessageSource.MSG_CREATE_BUNDLE:
                    createAndSendBundle(msg.replyTo);
                default:
                    break;
            }
            return false;
    ...
    private void createAndSendBundle(Messenger messenger) {
        mExecutor.execute(new Runnable() {
            @Override
            public void run() {
                int len = MessageSource.STRING_SOURCE_ARRAY.length;
                ...
                int index = random.nextInt(len);
                String ret = MessageSource.STRING_SOURCE_ARRAY[index];
                String str = "Give you a bundle data: " + ret;
                Message msg = new Message();
                msg.what = MessageSource.MSG_CREATE_BUNDLE;
                Bundle bundle = new Bundle();
                bundle.putString(MessageSource.MSG_KEY_BUNDLE,str);
                msg.setData(bundle);
                try {
                    messenger.send(msg);
    ...
    /**
     * Message 中的 obj 在跨进程传递中只支持使用 Parcelable 序列化的对象，其他对象无法传递(基础数据类型也一样)
     * @param type
     * @param str
     * @param messenger
     */
